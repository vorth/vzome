...
export class NumberController extends DefaultController {
    /*private*/ value: AlgebraicNumber;
    ...
    /*private*/ setValueByName(name: string) {
        const n: AlgebraicNumber = this.field.getNumberByName(name);
        if (n != null){
            this.setValue(n);
        }
    }
    ...
    public getProperty(property: string): string {
        switch((property)) {
        case "value":
            return this.value.toString(AlgebraicField.DEFAULT_FORMAT);
        case "evaluate":
    ...
    public setValue(value: AlgebraicNumber) {
        this.value = value;
    }
}
