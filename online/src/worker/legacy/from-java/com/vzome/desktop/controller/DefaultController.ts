...
export class DefaultController implements Controller {
    ...
    /*private*/ subcontrollers: java.util.Map<string, Controller>;
    ...
    properggties(): PropertyChangeSupport {
        return this.pcs;
    }
    ...
    public actionPerformed(source: any, action: string) {
        try {
            ...
            this.mErrors.reportError(Controller.UNKNOWN_ERROR_CODE, [ex, action]);
        }
    }
     ...
     */
    public paramActionPerformed(source: any, action: string, params: Properties) {
        try {
            ...
            console.error(ex.message, ex);
            this.mErrors.reportError(Controller.UNKNOWN_ERROR_CODE, [ex]);
        }
    }
    ...
    doAction(action: string) {
        if (this.mNextController != null)this.mNextController.doAction(action); else this.mErrors.reportError(Controller.UNKNOWN_ACTION, [action]);
    }
    ...
    doParamAction(action: string, params: Properties) {
        if (this.mNextController != null)this.mNextController.doParamAction(action, params); else this.mErrors.reportError(Controller.UNKNOWN_ACTION, [action]);
    }
    ...
    public doFileAction(command: string, file: File) {
        if (this.mNextController != null)this.mNextController.doFileAction(command, file); else this.mErrors.reportError(Controller.UNKNOWN_ACTION, [command]);
    }
    ...
    public doScriptAction(command: string, script: string) {
        if (this.mNextController != null)this.mNextController.doScriptAction(command, script); else this.mErrors.reportError(Controller.UNKNOWN_ACTION, [command]);
    }
     ...
     * @param {*} sub
     */
    public addSubController(name: string, sub: Controller) {
    ...
    public getSubController(name: string): Controller {
        const subc: Controller = this.subcontrollers.get(name);
    ...
    /*private*/ setNextController(controller: Controller, name: string) {
        this.name = name;
        ...
        if (this.mNextController != null && this.mNextController instanceof <any>DefaultController)this.mErrors = this.mNextController.mErrors;
    }

    firePropertyChange$java_beans_PropertyChangeEvent(event: PropertyChangeEvent) {
    ...
    public firePropertyChange(propName?: any, oldValue?: any, newValue?: any) {
        ...
        } else if (((propName != null && propName instanceof <any>PropertyChangeEvent) || propName === null) && oldValue === undefined && newValue === undefined) {
...
export namespace DefaultController {
    ...
    export class DefaultController$0 implements PropertyChangeListener {
        public __parent: any;
        /**
         * 
