...
export class LengthController extends DefaultController {
    /**
     ...
     */
    public static SCALE_OFFSET: number; public static SCALE_OFFSET_$LI$(): number { if (LengthController.SCALE_OFFSET == null) { LengthController.SCALE_OFFSET = Direction.USER_SCALE; }  return LengthController.SCALE_OFFSET; }

    currentScales: LengthController.ScaleController[];

    unitController: NumberController;

    /**
     * This is the internal factor applied, determined by the orbit, and fixed.
     */
    fixedFactor: AlgebraicNumber;

    /**
     * This is the user's basis for scale... when the slider is centered on "unit", this is the length value.
     */
    unitFactor: AlgebraicNumber;

    multiplier: number;

    standardUnitFactor: AlgebraicNumber;
    ...
    public getProperty(name: string): string {
        ...
        case "lengthText":
            {
                let result: AlgebraicNumber = this.unitFactor;
    ...
    applyScales(value: AlgebraicNumber): AlgebraicNumber {
        for(let i: number = 0; i < this.currentScales.length; i++) {{
    ...
    readable(unitFactor2: AlgebraicNumber): string {
     ...
     * @return {*}
     */
    public getValue(): AlgebraicNumber {
        let result: AlgebraicNumber = this.unitFactor['times$com_vzome_core_algebra_AlgebraicNumber'](this.fixedFactor);
     ...
     * @param {*} length
     */
    public setActualLength(length: AlgebraicNumber) {
        this.half = false;
        this.resetScales();
...
export namespace LengthController {
     ...
     * @class
     */
    export class ScaleController extends DefaultController {
        public __parent: any;
