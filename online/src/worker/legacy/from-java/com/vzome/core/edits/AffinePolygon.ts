...
export class AffinePolygon extends ChangeManifestations {
    /*private*/ errorMsg: java.lang.StringBuilder;

    /*private*/ field: AlgebraicField;

    /*private*/ chordRatio: AlgebraicNumber;
     ...
     * @return {*}
     */
    public static getPolygonModes(field: AlgebraicField): java.util.SortedMap<number, string> {
        ...
        if (field.getName() === ("heptagon")){
            ...
            const polygonSides: number = (<PolygonField><any>field).polygonSides();
    ...
    /*private*/ setMode(newMode: string) {
        ...
        if (this.mode != null && !/* isEmpty */(this.mode.length === 0)){
            try {
                ...
                if (ChangeSelection.logger_$LI$().isLoggable(java.util.logging.Level.WARNING)){
                    ChangeSelection.logger_$LI$().warning(this.getXmlElementName() + ": Invalid mode \"" + this.mode + "\".\nMode should be an integer greater than one.");
                }
            }
        }
        switch((this.nSides)) {
        ...
        default:
            if (this.nSides === 7 && (this.field.getName() === ("heptagon"))){
                ...
                const pField: PolygonField = <PolygonField><any>this.field;
                ...
                if (step * this.nSides === polygonSides){
                    const diag0: AlgebraicNumber = pField.getUnitDiagonal(step - 1);
                    const diag2: AlgebraicNumber = pField.getUnitDiagonal((step * 3) - 1);
    ...
    public perform() {
        ...
        let offset1: AlgebraicVector = strut1.getOffset();
        let offset2: AlgebraicVector = strut2.getOffset();
        if (AlgebraicVectors.areParallel(offset1, offset2)){
            this.fail(this.errorMsg.append("\nStruts are parallel or collinear.").toString());
        }
        let pCommon: Point = null;
        let pStart: Point = null;
        let pEnd: Point = null;
        const seg1: Segment = <Segment>strut1.getFirstConstruction();
        const seg2: Segment = <Segment>strut2.getFirstConstruction();
        {
            const s1: AlgebraicVector = strut1.getLocation();
            const e1: AlgebraicVector = strut1.getEnd();
            const s2: AlgebraicVector = strut2.getLocation();
            const e2: AlgebraicVector = strut2.getEnd();
            if (s1.equals(s2)){
        ...
        const vBegin: AlgebraicVector = pStart.getLocation();
        this.redo();
        const man0: Manifestation = this.manifestConstruction(pStart);
        const man1: Manifestation = this.manifestConstruction(seg1);
        if (this.nSides !== 2){
        ...
        for(let i: number = (this.nSides === 2) ? 1 : 2; i < this.nSides; i++) {{
            const translateByChordRatio: AlgebraicVector = offset2.scale(this.chordRatio);
            const pNew: Point = new TransformedPoint(new Translation(translateByChordRatio), pStart);
            const segNew: Segment = new SegmentJoiningPoints(pEnd, pNew);
            ...
            if (pNew.getLocation().equals(vBegin)){
                if (i + 1 < this.nSides){
                    if (ChangeSelection.logger_$LI$().isLoggable(java.util.logging.Level.INFO)){
                        ChangeSelection.logger_$LI$().info(this.getXmlElementName() + ": actual reps = " + (i + 1) + ", not " + this.nSides + " as specified.");
                    }
                }
            }
            pStart = pCommon;
