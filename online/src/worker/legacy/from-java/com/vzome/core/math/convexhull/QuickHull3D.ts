...
export class QuickHull3D {
    ...
    /*private*/ discardedFaces: Face[];

    /*private*/ maxVtxs: Vertex[];

    /*private*/ minVtxs: Vertex[];

    faces: java.util.Vector<Face>;

    /*private*/ newFaces: java.util.List<Face>;

    /*private*/ unclaimed: VertexList;

    /*private*/ claimed: VertexList;
    ...
    /*private*/ addPointToFace(vtx: Vertex, face: Face) {
        vtx.face = face;
    ...
    /*private*/ removePointFromFace(vtx: Vertex, face: Face) {
        if (vtx === face.outside){
    ...
    /*private*/ removeAllPointsFromFace(face: Face): Vertex {
        if (face.outside != null){
            let end: Vertex = face.outside;
    ...
    /*private*/ findHalfEdge(tail: Vertex, head: Vertex): HalfEdge {
        for(let index=this.faces.iterator();index.hasNext();) {
            let face = index.next();
            {
                const he: HalfEdge = face.findEdge(tail, head);
                if (he != null){
                    return he;
                }
            }
    ...
    setHull(coords: AlgebraicVector[], nump: number, faceIndices: number[][], numf: number) {
        this.initBuffers(nump);
        this.setPoints(coords, nump);
        this.computeMaxAndMin();
        for(let i: number = 0; i < numf; i++) {{
            const face: Face = Face.create(this.pointBuffer, faceIndices[i]);
            let he: HalfEdge = face.he0;
            do {{
                const heOpp: HalfEdge = this.findHalfEdge(he.head(), he.tail());
                if (heOpp != null){
    ...
    public build$java_util_Collection(points: java.util.Collection<AlgebraicVector>) {
    ...
    public build$com_vzome_core_algebra_AlgebraicVector_A(points: AlgebraicVector[]) {
        this.build$com_vzome_core_algebra_AlgebraicVector_A$int(points, points.length);
    }

    public build$com_vzome_core_algebra_AlgebraicVector_A$int(points: AlgebraicVector[], nump: number) {
        if (nump < 4){
    ...
    public build(points?: any, nump?: any) {
        if (((points != null && points instanceof <any>Array && (points.length == 0 || points[0] == null ||(points[0] != null && points[0] instanceof <any>AlgebraicVector))) || points === null) && ((typeof nump === 'number') || nump === null)) {
        ...
        } else if (((points != null && points instanceof <any>Array && (points.length == 0 || points[0] == null ||(points[0] != null && points[0] instanceof <any>AlgebraicVector))) || points === null) && nump === undefined) {
     ...
     * number of input points
     */
    public printPointSet(points: AlgebraicVector[], nump: number) {
        if (this.debug){
    ...
    initBuffers(nump: number) {
        if (this.pointBuffer.length < nump){
            const newBuffer: Vertex[] = (s => { let a=[]; while(s-->0) a.push(null); return a; })(nump);
    ...
    setPoints(pnts: AlgebraicVector[], nump: number) {
        this.pointBuffer = (s => { let a=[]; while(s-->0) a.push(null); return a; })(nump);
        for(let i: number = 0; i < nump; i++) {{
            this.pointBuffer[i] = new Vertex(pnts[i], i);
        };}
    }
    ...
    computeMaxAndMin() {
        ...
        const max: AlgebraicVector = this.pointBuffer[0].pnt;
        let maxx: number = max.getComponent(AlgebraicVector.X).evaluate();
        let maxy: number = max.getComponent(AlgebraicVector.Y).evaluate();
        let maxz: number = max.getComponent(AlgebraicVector.Z).evaluate();
        let minx: number = maxx;
        let miny: number = maxy;
        let minz: number = maxz;
        for(let i: number = 1; i < this.numPoints; i++) {{
            const pnt: AlgebraicVector = this.pointBuffer[i].pnt;
            const pntx: number = pnt.getComponent(AlgebraicVector.X).evaluate();
            const pnty: number = pnt.getComponent(AlgebraicVector.Y).evaluate();
            const pntz: number = pnt.getComponent(AlgebraicVector.Z).evaluate();
            if (pntx > maxx){
                maxx = pntx;
    ...
    createInitialSimplex() {
        ...
        const vtx: Vertex[] = [null, null, null, null];
        vtx[0] = this.maxVtxs[imax];
        vtx[1] = this.minVtxs[imax];
        let diff02: AlgebraicVector;
        let nrml: AlgebraicVector = null;
        let maxSqr: number = 0;
        const u01: AlgebraicVector = vtx[1].pnt.minus(vtx[0].pnt);
        for(let i: number = 0; i < this.numPoints; i++) {{
            diff02 = this.pointBuffer[i].pnt.minus(vtx[0].pnt);
            const xprod: AlgebraicVector = u01.cross(diff02);
            const lenSqr: AlgebraicNumber = xprod.dot(xprod);
        ...
        const res: AlgebraicVector = u01.scale(nrml.dot(u01));
        nrml = nrml.minus(res);
        let maxDist: number = 0.0;
        const d0: AlgebraicNumber = vtx[2].pnt.dot(nrml);
        for(let i: number = 0; i < this.numPoints; i++) {{
        ...
        const tris: Face[] = [null, null, null, null];
        if (vtx[3].pnt.dot(nrml)['minus$com_vzome_core_algebra_AlgebraicNumber'](d0).evaluate() < 0){
            tris[0] = Face.createTriangle$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex(vtx[0], vtx[1], vtx[2]);
            tris[1] = Face.createTriangle$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex(vtx[3], vtx[1], vtx[0]);
            tris[2] = Face.createTriangle$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex(vtx[3], vtx[2], vtx[1]);
            tris[3] = Face.createTriangle$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex(vtx[3], vtx[0], vtx[2]);
            for(let i: number = 0; i < 3; i++) {{
            ...
            tris[0] = Face.createTriangle$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex(vtx[0], vtx[2], vtx[1]);
            tris[1] = Face.createTriangle$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex(vtx[3], vtx[0], vtx[1]);
            tris[2] = Face.createTriangle$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex(vtx[3], vtx[1], vtx[2]);
            tris[3] = Face.createTriangle$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex(vtx[3], vtx[2], vtx[0]);
            for(let i: number = 0; i < 3; i++) {{
                const k: number = (i + 1) % 3;
        ...
        for(let i: number = 0; i < this.numPoints; i++) {{
            const v: Vertex = this.pointBuffer[i];
    ...
    public getVertices$(): AlgebraicVector[] {
        const vtxs: AlgebraicVector[] = (s => { let a=[]; while(s-->0) a.push(null); return a; })(this.numVertices);
    ...
    public getVertices$com_vzome_core_algebra_AlgebraicNumber_A(coords: AlgebraicNumber[]): number {
        for(let i: number = 0; i < this.numVertices; i++) {{
            const pnt: AlgebraicVector = this.pointBuffer[this.vertexPointIndices[i]].pnt;
            coords[i * 3 + 0] = pnt.getComponent(AlgebraicVector.X);
            coords[i * 3 + 1] = pnt.getComponent(AlgebraicVector.Y);
            coords[i * 3 + 2] = pnt.getComponent(AlgebraicVector.Z);
        };}
        return this.numVertices;
    }
    ...
    /*private*/ getFaceIndices(indices: number[], face: Face, flags: number) {
    ...
    resolveUnclaimedPoints(newFaces: java.util.List<Face>) {
        let vtxNext: Vertex = this.unclaimed.first();
        for(let vtx: Vertex = vtxNext; vtx != null; vtx = vtxNext) {{
            vtxNext = vtx.next;
    ...
    deleteFacePoints(face: Face, absorbingFace: Face) {
        const faceVtxs: Vertex = this.removeAllPointsFromFace(face);
        if (faceVtxs != null){
            if (absorbingFace == null){
                this.unclaimed.addAll(faceVtxs);
            } else {
                let vtxNext: Vertex = faceVtxs;
                for(let vtx: Vertex = vtxNext; vtx != null; vtx = vtxNext) {{
                    vtxNext = vtx.next;
    ...
    oppFaceDistance(he: HalfEdge): AlgebraicNumber {
        return he.face.distanceToPlane(he.opposite.face.getCentroid());
    }

    /*private*/ doAdjacentMerge(face: Face, mergeType: number): boolean {
        let hedge: HalfEdge = face.he0;
    ...
    calculateHorizon(eyePnt: AlgebraicVector, edge0: HalfEdge, face: Face, horizon: java.util.Vector<HalfEdge>) {
        ...
        const tolerance: number = 0;
        do {{
            const oppFace: Face = edge.oppositeFace();
    ...
    /*private*/ addAdjoiningFace(eyeVtx: Vertex, he: HalfEdge): HalfEdge {
        const face: Face = Face.createTriangle$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex$com_vzome_core_math_convexhull_Vertex(eyeVtx, he.tail(), he.head());
        this.faces.add(face);
        face.getEdge(-1).setOpposite(he.getOpposite());
        return face.getEdge(0);
    }

    addNewFaces(newFaces: java.util.List<Face>, eyeVtx: Vertex, horizon: java.util.Vector<HalfEdge>) {
        newFaces.clear();
        let hedgeSidePrev: HalfEdge = null;
        let hedgeSideBegin: HalfEdge = null;
        for(let index=horizon.iterator();index.hasNext();) {
            let horizonHe = index.next();
            {
                const hedgeSide: HalfEdge = this.addAdjoiningFace(eyeVtx, horizonHe);
                if (this.debug){
    ...
    nextPointToAdd(): Vertex {
        if (!this.claimed.isEmpty()){
            const eyeFace: Face = this.claimed.first().face;
            let eyeVtx: Vertex = null;
            let maxDist: number = 0;
            for(let vtx: Vertex = eyeFace.outside; vtx != null && vtx.face === eyeFace; vtx = vtx.next) {{
    ...
    addPointToHull(eyeVtx: Vertex) {
        const horizon: java.util.Vector<HalfEdge> = <any>(new java.util.Vector<any>());
    ...
    /*private*/ markFaceVertices(face: Face, mark: number) {
        const he0: HalfEdge = face.getFirstEdge();
        let he: HalfEdge = he0;
        do {{
            he.head().index = mark;
            he = he.next;
    ...
    reindexFacesAndVertices() {
        ...
        this.numFaces = 0;
        for(const it: java.util.Iterator<Face> = this.faces.iterator(); it.hasNext(); ) {{
            const face: Face = it.next();
            if (face.mark !== Face.VISIBLE){
                it.remove();
            } else {
        ...
        this.numVertices = 0;
        for(let i: number = 0; i < this.numPoints; i++) {{
            const vtx: Vertex = this.pointBuffer[i];
            if (vtx.index === 0){
    ...
    checkFaceConvexity(face: Face, ps: java.io.PrintStream): boolean {
        let he: HalfEdge = face.he0;
        do {{
            face.checkConsistency();
            let dist: AlgebraicNumber = this.oppFaceDistance(he);
            if (!dist.isZero()){
                if (ps != null){
    ...
    public check(ps: java.io.PrintStream): boolean {
        ...
        for(let i: number = 0; i < this.numPoints; i++) {{
            ...
            for(let index=this.faces.iterator();index.hasNext();) {
                ...
                {
                    if (face.mark === Face.VISIBLE){
                        const dist: AlgebraicNumber = face.distanceToPlane(pnt);
                        if (!dist.isZero()){
                            if (ps != null){
