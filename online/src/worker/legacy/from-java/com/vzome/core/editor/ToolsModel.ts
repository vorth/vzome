...
import { SelectToolParameters } from "./SelectToolParameters.js";
...
import { BookmarkTool } from "../tools/BookmarkTool.js";
...
export class ToolsModel extends java.util.TreeMap<string, CoreTool> implements ApiTool.Source {
    ...
    public constructor(context: Context, originPoint: Point) {
        super();
     ...
     * @return {CoreTool}
     */
    public put(key: string, tool: CoreTool): CoreTool {
        const result: CoreTool = super.put(key, tool);
        ...
        if (!tool.isPredefined() && !tool.isHidden()){
            if (tool.getCategory() === BookmarkTool.ID){
    ...
    public getToolIDs(bookmarks: boolean): string[] {
        ...
        const tools: java.util.List<CoreTool> = <any>(new java.util.ArrayList<any>());
        for(let index=ids.iterator();index.hasNext();) {
            let id = index.next();
            {
                const tool: CoreTool = this.get(id);
    ...
    public getAllCustomToolIDs(bookmarks: boolean): string[] {
        ...
        for(let index=this.values().iterator();index.hasNext();) {
            let tool = index.next();
            if (!tool.isPredefined() && (tool.getCategory() === BookmarkTool.ID) === bookmarks)tools.add(tool);
        }
        tools.sort(<any>(((funcInst: any) => { if (funcInst == null || typeof funcInst == 'function') { return funcInst } return (arg0, arg1) =>  (funcInst['compare'] ? funcInst['compare'] : funcInst) .call(funcInst, arg0, arg1)})(ToolsModel.TOOLBAR_ORDER_$LI$())));
        const result: java.util.List<string> = <any>(new java.util.ArrayList<any>());
        for(let index=tools.iterator();index.hasNext();) {
    ...
    static TOOLBAR_ORDER: java.util.Comparator<CoreTool>; public static TOOLBAR_ORDER_$LI$(): java.util.Comparator<CoreTool> { if (ToolsModel.TOOLBAR_ORDER == null) { ToolsModel.TOOLBAR_ORDER = (a, b) => {
    ...
    public reorderTools(bookmarks: boolean, orderedIds: string[]) {
        for(let i: number = 0; i < orderedIds.length; i++) {{
            const tool: CoreTool = this.get(orderedIds[i]);
    ...
    public createEdit(className: string): UndoableEdit {
        switch((className)) {
        case "ToolApplied":
    ...
    public applyTool(tool: CoreTool, selectInputs: boolean, deleteInputs: boolean, createOutputs: boolean, selectOutputs: boolean, copyColors: boolean) {
        const edit: UndoableEdit = new ApplyTool(this, tool, selectInputs, deleteInputs, createOutputs, selectOutputs, true, copyColors);
        this.getContext().performAndRecord(edit);
    }

    public selectToolParameters(tool: CoreTool) {
        const edit: UndoableEdit = new SelectToolParameters(this, tool);
        this.getContext().performAndRecord(edit);
    }
     ...
     * @return {CoreTool}
     */
    public getPredefinedTool(id: string): CoreTool {
        return this.get(id);
    }
    ...
    public getXml(doc: Document): Element {
        const result: Element = doc.createElement("Tools");
        for(let index=this.values().iterator();index.hasNext();) {
            let tool = index.next();
            if (!tool.isPredefined()){
                const toolElem: Element = doc.createElement("Tool");
                DomUtils.addAttribute(toolElem, "id", tool.getId());
                DomUtils.addAttribute(toolElem, "label", tool.getLabel());
                if (tool.isHidden())DomUtils.addAttribute(toolElem, "hidden", "true");
                if (tool.getOrder() >= 0)DomUtils.addAttribute(toolElem, "order", /* toString */(''+(tool.getOrder())));
    ...
    loadFromXml(xml: Element) {
        ...
        for(let i: number = 0; i < nodes.getLength(); i++) {{
            ...
            if (node != null && (node.constructor != null && node.constructor["__interfaces"] != null && node.constructor["__interfaces"].indexOf("org.w3c.dom.Element") >= 0)){
                const toolElem: Element = <Element><any>node;
    ...
    public setConfiguration(tool: CoreTool) {
    ...
    public unhideTool(tool: CoreTool) {
        this.pcs.firePropertyChange$java_lang_String$java_lang_Object$java_lang_Object("tool.instances", null, tool);
        if (!tool.isPredefined()){
            if (tool.getCategory() === BookmarkTool.ID){
