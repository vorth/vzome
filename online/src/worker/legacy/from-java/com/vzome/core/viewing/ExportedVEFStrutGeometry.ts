...
export class ExportedVEFStrutGeometry implements StrutGeometry {
    ...
    public getStrutPolyhedron(length: AlgebraicNumber): Polyhedron {
        const tipVertex: AlgebraicVector = this.prototypeVector.scale(length);
        let maxNonTipDistance: number = 0;
        let minTipDistance: number = tipVertex.toRealVector().length();
        const midpoint: AlgebraicVector = tipVertex.scale(this.field['createRational$long$long'](1, 2));
        ...
        const result: Polyhedron = new Polyhedron(this.field);
        for(let i: number = 0; i < this.prototypeVertices.size(); i++) {{
            let vertex: AlgebraicVector = this.prototypeVertices.get(i);
    ...
    public setShortGeometry(shortGeometry: StrutGeometry) {
