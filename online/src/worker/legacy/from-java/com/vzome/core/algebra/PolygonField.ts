...
import { ParameterizedField } from "./ParameterizedField.js";
 ...
 * @param {*} factory
 * @class
 * @extends ParameterizedField
 */
export class PolygonField extends ParameterizedField {
    ...
    public getNormalizerMatrix(nSides: number): number[][] {
        ...
        const primaryDiags: BigRational[][] = <any> (function(dims) { let allocate = function(dims) { if (dims.length === 0) { return null; } else { let array = []; for(let i = 0; i < dims[0]; i++) { array.push(allocate(dims.slice(1))); } return array; }}; return allocate(dims);})([nEquations, nPrimaryDiags]);
        const secondaryDiags: BigRational[][] = <any> (function(dims) { let allocate = function(dims) { if (dims.length === 0) { return null; } else { let array = []; for(let i = 0; i < dims[0]; i++) { array.push(allocate(dims.slice(1))); } return array; }}; return allocate(dims);})([nEquations, nSecondaryDiags]);
        ...
        const rank: number = Fields.gaussJordanReduction$com_vzome_core_algebra_Fields_Element_A_A$com_vzome_core_algebra_Fields_Element_A_A(secondaryDiags, primaryDiags);
        ...
        for(let r: number = 0; r < rank; r++) {{
            for(let c: number = nPrimaryDiags - 1; c >= 0; c--) {{
                const bigTerm: BigRational = primaryDiags[rank - 1 - r][nPrimaryDiags - 1 - c];
    ...
    /*private*/ goldenRatio: AlgebraicNumber;
     ...
     * @return {*}
     */
    public getUnitDiagonal(n: number): AlgebraicNumber {
    ...
    /*private*/ getRoot2(): AlgebraicNumber {
        return this.getDiagonalRatio$int(4);
    }

    /*private*/ getRoot3(): AlgebraicNumber {
        return this.getDiagonalRatio$int(6);
    }

    /*private*/ getRoot6(): AlgebraicNumber {
        const r3: AlgebraicNumber = this.getNumberByName("root3");
        if (r3 != null){
            const r2: AlgebraicNumber = this.getNumberByName("root2");
            return r2 == null ? null : r2['times$com_vzome_core_algebra_AlgebraicNumber'](r3);
        }
        return null;
    }

    /*private*/ getRoot7(): AlgebraicNumber {
        if (this.__polygonSides % 14 === 0){
            const n: number = (this.__polygonSides / 14|0);
            const d0: AlgebraicNumber = this.getUnitDiagonal((1 * n) - 1).negate();
            const d1: AlgebraicNumber = this.getUnitDiagonal((2 * n) - 1);
            const d2: AlgebraicNumber = this.getUnitDiagonal((3 * n) - 1);
            const d3: AlgebraicNumber = this.getUnitDiagonal((4 * n) - 1);
            const d4: AlgebraicNumber = this.getUnitDiagonal((5 * n) - 1);
            const d5: AlgebraicNumber = this.getUnitDiagonal((6 * n) - 1);
            const cotA: AlgebraicNumber = d4.dividedBy(d1);
            const cotB: AlgebraicNumber = d2.dividedBy(d3);
            const cotC: AlgebraicNumber = d0.dividedBy(d5);
            return cotA['plus$com_vzome_core_algebra_AlgebraicNumber'](cotB)['plus$com_vzome_core_algebra_AlgebraicNumber'](cotC);
        }
        return null;
    }

    /*private*/ getRoot10(): AlgebraicNumber {
        const r5: AlgebraicNumber = this.getNumberByName("root5");
        if (r5 != null){
            const r2: AlgebraicNumber = this.getNumberByName("root2");
    ...
    /*private*/ getDiagonalRatio$int(divisor: number): AlgebraicNumber {
        return this.getDiagonalRatio$int$int(divisor, 2);
    }

    public getDiagonalRatio$int$int(divisor: number, step: number): AlgebraicNumber {
        if (this.__polygonSides % divisor === 0 && step > 1 && step * 2 <= this.__polygonSides){
            const n: number = (this.__polygonSides / divisor|0);
            const denominator: AlgebraicNumber = this.getUnitDiagonal(n - 1);
            const numerator: AlgebraicNumber = this.getUnitDiagonal((step * n) - 1);
            return numerator.dividedBy(denominator);
        }
        return null;
    }

    public getDiagonalRatio(divisor?: any, step?: any): AlgebraicNumber {
