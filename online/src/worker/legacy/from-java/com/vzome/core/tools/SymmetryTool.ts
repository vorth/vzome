...
export class SymmetryTool extends TransformationTool {
    ...
    checkSelection(prepareTool: boolean): string {
        ...
        if (!this.isAutomatic())for(let index=this.mSelection.iterator();index.hasNext();) {
            let man = index.next();
            {
                ...
                if (man != null && (man.constructor != null && man.constructor["__interfaces"] != null && man.constructor["__interfaces"].indexOf("com.vzome.core.model.Connector") >= 0)){
                    ...
                    if (axis != null)correct = false; else axis = <Strut><any>man;
        ...
        let closure: number[] = this.symmetry.subgroup(Symmetry.TETRAHEDRAL);
        ...
        case "icosahedral":
            ...
            case "tetrahedral":
                ...
                if (axis == null){
                    ...
                    const icosa: IcosahedralSymmetry = <IcosahedralSymmetry><any>this.symmetry;
                    const zone: Axis = icosa.getAxis$com_vzome_core_algebra_AlgebraicVector(axis.getOffset());
                    if (zone == null)return "selected alignment strut is not a tetrahedral axis.";
                    const allowYellow: boolean = prepareTool;
                    closure = icosa.subgroup$java_lang_String$com_vzome_core_math_symmetry_Axis$boolean(Symmetry.TETRAHEDRAL, zone, allowYellow);
                ...
                if (prepareTool){
                    ...
                    for(let i: number = 0; i < order - 1; i++) {this.transforms[i] = new SymmetryTransformation(this.symmetry, closure[i + 1], center);}
                }
                break;
            case "octahedral":
                let orientation: AlgebraicMatrix = null;
                if (!correct)return "no unique alignment strut selected.";
                if (axis == null){
                    if (!prepareTool)return "no aligment strut selected.";
                } else {
                ...
                if (prepareTool){
                    ...
                    const octa: OctahedralSymmetry = (this.symmetry != null && this.symmetry instanceof <any>OctahedralSymmetry) ? <OctahedralSymmetry><any>this.symmetry : new OctahedralSymmetry(this.symmetry.getField());
                    const order: number = octa.getChiralOrder();
                    this.transforms = (s => { let a=[]; while(s-->0) a.push(null); return a; })(order - 1);
                    for(let i: number = 0; i < order - 1; i++) {{
                        let matrix: AlgebraicMatrix = octa.getMatrix(i + 1);
                        matrix = orientation.times(matrix.times(inverse));
                        this.transforms[i] = new MatrixTransformation(matrix, center.getLocation());
                    };}
                }
                break;
            default:
    ...
    /*private*/ prepareFullSymmetry(center: Point) {
        const order: number = this.symmetry.getChiralOrder();
        this.transforms = (s => { let a=[]; while(s-->0) a.push(null); return a; })(order - 1);
        for(let i: number = 0; i < order - 1; i++) {this.transforms[i] = new SymmetryTransformation(this.symmetry, i + 1, center);}
    }
