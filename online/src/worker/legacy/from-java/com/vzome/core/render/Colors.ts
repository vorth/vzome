...
export class Colors implements java.lang.Iterable<string> {
    ...
    /*private*/ mColors: java.util.Map<string, Color>;

    /*private*/ mListeners: java.util.List<Colors.Changes>;

    /*private*/ properties: Properties;

    public addColor(name: string, color: Color) {
    ...
    public getColorPref(name: string): Color {
        const percents: number[] = this.getVectorPref("color.percent." + name);
        if (percents !== Colors.NO_VECTOR_$LI$()){
            return new Color(Math.round((<any>Math).fround((<any>Math).fround(percents[0] * 255) / 100)), Math.round((<any>Math).fround((<any>Math).fround(percents[1] * 255) / 100)), Math.round((<any>Math).fround((<any>Math).fround(percents[2] * 255) / 100)));
        }
        const pref: string = this.properties.getProperty("color." + name);
        return Colors.parseColor(pref);
    }

    public static parseColor(colorString: string): Color {
        if (colorString == null || (colorString === ("")))return Color.WHITE_$LI$();
    ...
    public getColor(name: string): Color {
        let color: Color = this.mColors.get(name);
        if (color == null){
...
export namespace Colors {
    ...
    export interface Changes {
        colorAdded(name: string, color: Color);

        colorChanged(name: string, newColor: Color);
    }
}
