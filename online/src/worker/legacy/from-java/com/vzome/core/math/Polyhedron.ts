...
import { Direction } from "./symmetry/Direction.js";
...
export class Polyhedron implements java.lang.Cloneable {
    ...
    m_vertices: java.util.Map<AlgebraicVector, number>;

    m_vertexList: java.util.List<AlgebraicVector>;
     ...
     * outward.
     * @return
     * @param {AlgebraicMatrix} reflection
     * @return {Polyhedron}
     */
    public getEvilTwin(reflection: AlgebraicMatrix): Polyhedron {
    ...
    public addVertex(location: AlgebraicVector) {
     ...
     * @return {number}
     */
    public addIndexedVertex(location: AlgebraicVector): number {
    ...
    public setOrbit(orbit: Direction) {
        this.orbit = orbit;
    }

    public setLength(length: AlgebraicNumber) {
        this.length = length;
    }
    ...
    public getVertexList(): java.util.List<AlgebraicVector> {
    ...
    public getTriangleVertices(): java.util.List<RealVector> {
        const result: java.util.ArrayList<RealVector> = <any>(new java.util.ArrayList<any>());
        for(let index=this.m_faces.iterator();index.hasNext();) {
            let face = index.next();
            {
                for(let index=face.getTriangles().iterator();index.hasNext();) {
                    let triangle = index.next();
                    {
                        for(let loopIndex = 0; loopIndex < triangle.vertices.length; loopIndex++) {
                            ...
                            {
                                const vertex: AlgebraicVector = this.m_vertexList.get(index);
    ...
    public getNormals(): java.util.List<RealVector> {
        const result: java.util.ArrayList<RealVector> = <any>(new java.util.ArrayList<any>());
        for(let index=this.m_faces.iterator();index.hasNext();) {
            let face = index.next();
            {
                const normal: RealVector = face.getNormal(this.m_vertexList).toRealVector();
                for(let index=face.getTriangles().iterator();index.hasNext();) {
                    let triangle = index.next();
                    {
                        result.add(normal);
    ...
    export class Face extends java.util.ArrayList<number> {
        ...
        public getVertex(index: number): number {
            if (index >= this.size()){
                ...
                Polyhedron.logger_$LI$().severe(msg);
                throw new java.lang.IllegalStateException(msg);
            }
            return this.get(index);
        }

        public getTriangles(): java.util.List<Polyhedron.Face.Triangle> {
            ...
            for(let j: number = 0; j < arity; j++) {{
                ...
                if (v0 === -1){
                    ...
                    const triangle: Polyhedron.Face.Triangle = new Polyhedron.Face.Triangle(this, v0, v1, index);
                    result.add(triangle);
                    v1 = index;
                }
            };}
        ...
        public getNormal(vertices: java.util.List<AlgebraicVector>): AlgebraicVector {
            return AlgebraicVectors.getNormal$com_vzome_core_algebra_AlgebraicVector$com_vzome_core_algebra_AlgebraicVector$com_vzome_core_algebra_AlgebraicVector(vertices.get(this.getVertex(0)), vertices.get(this.getVertex(1)), vertices.get(this.getVertex(2)));
        }
        ...
        public equals(other: any): boolean {
            ...
            if (!(other != null && other instanceof <any>Polyhedron.Face))return false;
