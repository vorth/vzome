...
export class AlgebraicVector implements java.lang.Comparable<AlgebraicVector> {
    ...
    /*private*/ coordinates: AlgebraicNumber[];
    ...
    public compareTo(other: AlgebraicVector): number {
        ...
        for(let i: number = 0; i < this.coordinates.length; i++) {{
            const n1: AlgebraicNumber = this.coordinates[i];
            const n2: AlgebraicNumber = other.coordinates[i];
    ...
    public toRealVector(): RealVector {
    ...
    public toASCIIString(): string {
        return this.getVectorExpression$int(AlgebraicField.EXPRESSION_FORMAT);
    }
    ...
    public toParsableString(): string {
        return this.getVectorExpression$int(AlgebraicField.ZOMIC_FORMAT);
    }

    public toString(format: number = AlgebraicField.DEFAULT_FORMAT): string {
        return this.getVectorExpression$int(format);
    }

    public getComponent(i: number): AlgebraicNumber {
        return this.coordinates[i];
    }

    public getComponents(): AlgebraicNumber[] {
        return this.coordinates;
    }

    public setComponent(component: number, coord: AlgebraicNumber): AlgebraicVector {
        this.coordinates[component] = coord;
        return this;
    }

    public negate(): AlgebraicVector {
        const result: AlgebraicNumber[] = (s => { let a=[]; while(s-->0) a.push(null); return a; })(this.coordinates.length);
    ...
    public scale(scale: AlgebraicNumber): AlgebraicVector {
        const result: AlgebraicNumber[] = (s => { let a=[]; while(s-->0) a.push(null); return a; })(this.coordinates.length);
        for(let i: number = 0; i < result.length; i++) {{
    ...
    public getVectorExpression$java_lang_StringBuffer$int(buf: java.lang.StringBuffer, format: number) {
        if (format === AlgebraicField.DEFAULT_FORMAT)buf.append("(");
        for(let i: number = 0; i < this.coordinates.length; i++) {{
            if (i > 0)if (format === AlgebraicField.VEF_FORMAT || format === AlgebraicField.ZOMIC_FORMAT)buf.append(" "); else buf.append(", ");
    ...
    public dot(that: AlgebraicVector): AlgebraicNumber {
        let result: AlgebraicNumber = this.field.zero();
    ...
    public getLength(unit: AlgebraicVector): AlgebraicNumber {
        for(let i: number = 0; i < this.coordinates.length; i++) {{
