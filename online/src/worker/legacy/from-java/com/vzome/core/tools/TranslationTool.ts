...
import { PointToPointTranslation } from "../construction/PointToPointTranslation.js";
...
export class TranslationTool extends TransformationTool {
    ...
    checkSelection(prepareTool: boolean): string {
        ...
        if (!this.isAutomatic())for(let index=this.mSelection.iterator();index.hasNext();) {
            let man = index.next();
            {
                ...
                if (man != null && (man.constructor != null && man.constructor["__interfaces"] != null && man.constructor["__interfaces"].indexOf("com.vzome.core.model.Connector") >= 0)){
                    ...
                    if (p1 == null)p1 = <Point>(<Connector><any>man).getFirstConstruction(); else p2 = <Point>(<Connector><any>man).getFirstConstruction();
                } else if (!prepareTool){
        ...
        if (p1 == null){
            if (this.isAutomatic() || this.isPredefined()){
                ...
                let xAxis: AlgebraicVector = field.basisVector(3, AlgebraicVector.X);
                let scale: AlgebraicNumber = field['createPower$int'](3);
        ...
        if (prepareTool){
            ...
            this.transforms[0] = new PointToPointTranslation(p1, p2);
        }
        return null;
    }
