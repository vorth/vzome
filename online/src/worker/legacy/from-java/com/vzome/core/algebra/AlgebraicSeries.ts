...
export class AlgebraicSeries {
    public series: AlgebraicNumber[];

    public constructor(field: AbstractAlgebraicField, power: number) {
        if (this.series === undefined) { this.series = null; }
        let sequence: java.util.List<number> = <any>(new java.util.ArrayList<any>());
        sequence.add(0);
        let divisor: AlgebraicNumber = field.getUnitTerm(1);
        ...
        for(let index=sequence.iterator();index.hasNext();) {
            ...
            {
                const prev: AlgebraicNumber = this.series[prevIndex++];
                const step: AlgebraicNumber = field.getUnitTerm(integer).dividedBy(divisor);
    ...
    public nearestAlgebraicNumber(target: number): AlgebraicNumber {
        ...
        const positive: AlgebraicNumber = this.checkRange(0, this.series.length - 1, target);
        if (negative)return positive.negate(); else return positive;
    }

    /*private*/ checkRange(minIndex: number, maxIndex: number, target: number): AlgebraicNumber {
    ...
    public toString(): string {
        ...
        for(let index = 0; index < this.series.length; index++) {
            ...
            {
                result += algebraicNumber.toString(AlgebraicField.DEFAULT_FORMAT) + ", ";
            }
        }
        return result;
    }
