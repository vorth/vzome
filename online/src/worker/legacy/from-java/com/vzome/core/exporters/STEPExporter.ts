 ...
 */
export class STEPExporter extends GeometryExporter {
    ...
    public doExport(file: File, writer: java.io.Writer, height: number, width: number) {
        ...
        const preamble: string = ResourceLoader.loadStringResource(STEPExporter.PREAMBLE_FILE);
        ...
        Manifestations.sortVertices(this.mModel.getManifestations(), vertices);
        ...
        const realVectors: java.util.ArrayList<RealVector> = <any>(new java.util.ArrayList<any>());
        for(let index1=sortedVertexList.iterator();index1.hasNext();) {
            let gv = index1.next();
            {
                const v: RealVector = this.mModel.renderVector(gv).scale(inchScaling);
                realVectors.add(v);
        ...
        for(let index1=this.mModel.getManifestations().iterator();index1.hasNext();) {
            let man = index1.next();
            {
                if (man != null && (man.constructor != null && man.constructor["__interfaces"] != null && man.constructor["__interfaces"].indexOf("com.vzome.core.model.Panel") >= 0)){
                    ...
                    let point1: number = 0;
                    let dir1: RealVector = null;
                    let dir2: RealVector = null;
                    for(let j: number = 0; j < arity; j++) {{
                        let vindex: number = vIndices[j];
                        const rv1index: number = vindex;
                        let rv1: RealVector = realVectors.get(rv1index);
                        ...
                        let rv2: RealVector = realVectors.get(rv2index);
