...
export abstract class SideEffects extends UndoableEdit {
    /**
     * 
     * @param {*} doc
     * @return {*}
    ...
    public getDetailXml(doc: Document): Element {
        const result: Element = this.getXml(doc);
        const effects: Element = doc.createElement("effects");
        for(let index=this.mItems.iterator();index.hasNext();) {
            let se = index.next();
            {
                if (se != null){
                    const effect: Element = se.getXml(doc);
                    if (effect != null){
    ...
    /*private*/ mItems: java.util.List<SideEffect>;
    ...
    public redo() {
        for(let i: number = this.redone; i < this.mItems.size(); i++) {{
            const se: SideEffect = this.mItems.get(i);
            if (se != null)se.redo();
        };}
    ...
    public undo() {
        for(let i: number = this.mItems.size(); i > 0; i--) {{
            const se: SideEffect = this.mItems.get(i - 1);
            if (se != null)se.undo();
        };}
        this.redone = 0;
    }

    getEffects(): java.util.Iterator<SideEffect> {
