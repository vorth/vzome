...
export class LinearMapTool extends TransformationTool {
    ...
    public constructor(name: string, tools: ToolsModel, originalScaling: boolean) {
        super(name, tools);
    ...
    checkSelection(prepareTool: boolean): string {
        const oldBasis: Segment[] = [null, null, null];
        const newBasis: Segment[] = [null, null, null];
        let index: number = 0;
        ...
        if (prepareTool){
            ...
            if (index === 6)this.transforms[0] = new ChangeOfBasis(oldBasis, newBasis, center); else this.transforms[0] = new ChangeOfBasis(oldBasis[0], oldBasis[1], oldBasis[2], center, this.originalScaling);
        }
        return null;
    }
