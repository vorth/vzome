 ...
 * @class
 */
export class Direction implements java.lang.Comparable<Direction>, java.lang.Iterable<Axis> {
    /**
     * 
     * @return {number}
     */
    ...
    /*private*/ zoneNames: Axis[][][];

    /*private*/ zoneVectors: java.util.Map<string, Axis>;

    /*private*/ mSymmetryGroup: Symmetry;

    /*private*/ mPrototype: AlgebraicVector;
    ...
    public scales: AlgebraicNumber[];

    /*private*/ unitLength: AlgebraicNumber;

    /*private*/ unitLengthReciprocal: AlgebraicNumber;
    ...
    public constructor(name: string, group: Symmetry, prototype: number, rotatedPrototype: number, vector: AlgebraicVector, isStd: boolean) {
    ...
    /*private*/ getZoneVectors(): java.util.Map<string, Axis> {
        if (this.zoneVectors.isEmpty()){
            ...
            for(let i: number = 0; i < this.mSymmetryGroup.getChiralOrder(); i++) {{
                const transform: AlgebraicMatrix = this.mSymmetryGroup.getMatrix(i);
                const perm: Permutation = this.mSymmetryGroup.getPermutation(i);
                const j: number = perm.mapIndex(this.prototype);
                const rotated: number = perm.mapIndex(this.rotatedPrototype);
                const normal: AlgebraicVector = transform.timesColumn(this.mPrototype);
    ...
    public getPrototype(): AlgebraicVector {
     ...
     * @return {*}
     */
    public iterator(): java.util.Iterator<Axis> {
    ...
    public getCanonicalName(): string {
        if (this.canonicalName == null){
            const canonicalAxis: Axis = this.getCanonicalAxis(0, 0);
            const vector: AlgebraicVector = canonicalAxis.normal();
            const x: AlgebraicNumber = vector.getComponent(0);
            let y: AlgebraicNumber = vector.getComponent(1);
            let z: AlgebraicNumber = vector.getComponent(2).negate();
            if (x.isZero()){
    ...
    public getAxis$com_vzome_core_algebra_AlgebraicVector(vector: AlgebraicVector): Axis {
        for(let index=this.getZoneVectors().values().iterator();index.hasNext();) {
            let axis = index.next();
            {
                const normal: AlgebraicVector = axis.normal();
                if (AlgebraicVectors.areParallel(normal, vector)){
                    const dotProd: AlgebraicNumber = normal.dot(vector);
                    if (dotProd.evaluate() > 0){
                        return axis;
                    } else {
                        const principalReflection: AlgebraicMatrix = this.mSymmetryGroup.getPrincipalReflection();
    ...
    public getAxis$com_vzome_core_math_RealVector(vector: RealVector): Axis {
    ...
    getChiralAxis(vector: RealVector): Axis {
        if (RealVector.ORIGIN_$LI$().equals(vector)){
            return null;
        }
        const vectorLength: number = vector.length();
        const checked: java.util.Set<Axis> = <any>(new java.util.HashSet<any>());
        let closestOrientation: number = 0;
        let closestSense: number = Symmetry.PLUS;
        let closestAxis: Axis = this.getCanonicalAxis(Symmetry.PLUS, 0);
        checked.add(closestAxis);
        let axisV: RealVector = closestAxis.normal().toRealVector();
        ...
        while((!finished)) {{
            ...
            for(let index = 0; index < incidentOrientations.length; index++) {
                ...
                {
                    const neighbor: Axis = this.getCanonicalAxis(reverseSense, i);
    ...
    getAxisBruteForce(vector: RealVector): Axis {
        let closestAxis: Axis = null;
    ...
    public getAxis$int$int(sense: number, index: number): Axis {
        return this.getAxis$int$int$boolean(sense, index, true);
    }

    public getAxis$int$int$boolean(sense: number, index: number, outbound: boolean): Axis {
        this.getZoneVectors();
        return this.zoneNames[outbound ? 1 : 0][sense][index];
    }

    public getAxis(sense?: any, index?: any, outbound?: any): Axis {
        ...
        } else if (((sense != null && sense instanceof <any>AlgebraicVector) || sense === null) && index === undefined && outbound === undefined) {
            return <any>this.getAxis$com_vzome_core_algebra_AlgebraicVector(sense);
        } else if (((sense != null && sense instanceof <any>RealVector) || sense === null) && index === undefined && outbound === undefined) {
     ...
     * @return {Axis}
     */
    public getCanonicalAxis(sense: number, index: number): Axis {
        if (this.needsCanonicalization){
            const treatedAs0: Axis = this.getAxisBruteForce(RealVector.DIRECTION_0_$LI$());
            this.canonicalize = treatedAs0.getOrientation();
            if (treatedAs0.getSense() === Symmetry.MINUS)this.canonicalize *= -1;
            this.needsCanonicalization = false;
        }
        if (this.canonicalize !== 0){
            if (this.canonicalize < 0)sense = (sense + 1) % 2;
            const target: Permutation = this.mSymmetryGroup.getPermutation(index);
    ...
    public createAxis$int$int$int_A_A(orientation: number, rotation: number, norm: number[][]) {
        const aNorm: AlgebraicVector = this.mSymmetryGroup.getField().createVector(norm);
    ...
    public createAxis(orientation?: any, rotation?: any, norm?: any) {
        ...
        } else if (((typeof orientation === 'number') || orientation === null) && ((typeof rotation === 'number') || rotation === null) && ((norm != null && norm instanceof <any>AlgebraicVector) || norm === null)) {
            return <any>this.createAxis$int$int$com_vzome_core_algebra_AlgebraicVector(orientation, rotation, norm);
        } else throw new Error('invalid overload');
    }

    public createAxis$int$int$com_vzome_core_algebra_AlgebraicVector(orientation: number, rotation: number, norm: AlgebraicVector) {
        let perm: Permutation = this.mSymmetryGroup.getPermutation(rotation);
        this.recordZone(this, orientation, Symmetry.PLUS, rotation, perm, norm, true);
        const inversion: AlgebraicMatrix = this.mSymmetryGroup.getPrincipalReflection();
        if (inversion == null){
            if (perm != null)perm = perm.inverse();
            this.recordZone(this, orientation, Symmetry.MINUS, rotation, perm, norm.negate(), true);
        } else {
            let reverseRotation: number = rotation;
            let reversePerm: Permutation = perm;
            if (perm != null){
                reversePerm = perm.inverse();
                reverseRotation = perm.mapIndex(0);
            }
            this.recordZone(this, orientation, Symmetry.PLUS, reverseRotation, reversePerm, norm.negate(), false);
            let reflectedNorm: AlgebraicVector = inversion.timesColumn(norm);
            this.recordZone(this, orientation, Symmetry.MINUS, reverseRotation, reversePerm, reflectedNorm, true);
            reflectedNorm = reflectedNorm.negate();
            this.recordZone(this, orientation, Symmetry.MINUS, rotation, perm, reflectedNorm, false);
        }
    }

    /*private*/ recordZone(dir: Direction, orientation: number, sense: number, rotation: number, rotPerm: Permutation, normal: AlgebraicVector, outbound: boolean) {
        let zone: Axis = this.zoneVectors.get(normal.toString());
        if (zone == null){
            ...
            if (outbound && !zone.isOutbound()){
                ...
                if (Direction.logger_$LI$().isLoggable(java.util.logging.Level.FINEST))Direction.logger_$LI$().finest("zone " + zone.toString() + " aliased as " + ((sense === Axis.MINUS) ? "-" : "") + orientation + (outbound ? "" : "i"));
    ...
    public setUnitLength(unitLength: AlgebraicNumber) {
        this.unitLength = unitLength;
        this.unitLengthReciprocal = unitLength.reciprocal();
    }

    public getUnitLength(): AlgebraicNumber {
    ...
    public getLengthInUnits(rawLength: AlgebraicNumber): AlgebraicNumber {
        const field: AlgebraicField = this.mSymmetryGroup.getField();
        const scaledLength: AlgebraicNumber = rawLength['times$com_vzome_core_algebra_AlgebraicNumber'](field['createPower$int'](-Direction.USER_SCALE));
    ...
    public getLengthName(length: AlgebraicNumber): string {
        for(let i: number = 0; i < this.scales.length; i++) {{
    ...
    public getLengthExpression(buf: java.lang.StringBuffer, length: AlgebraicNumber) {
