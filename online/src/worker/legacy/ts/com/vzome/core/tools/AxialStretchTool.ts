...
export class AxialStretchTool extends TransformationTool {
    ...
    public constructor(id: string, symmetry: IcosahedralSymmetry, tools: ToolsModel, stretch: boolean, red: boolean, first: boolean, category: string) {
        super(id, tools);
    ...
    checkSelection(prepareTool: boolean): string {
        ...
        const zone: Axis = this.symmetry.getAxis$com_vzome_core_algebra_AlgebraicVector(axis.getOffset());
        if (zone == null)return "Selected alignment strut is not an appropriate axis.";
        let o0: AlgebraicVector;
        let o1: AlgebraicVector;
        let o2: AlgebraicVector;
        let n0: AlgebraicVector;
        let n1: AlgebraicVector;
        let n2: AlgebraicVector;
        switch((zone.getDirection().getName())) {
        case "yellow":
            {
                if (this.red)return "A red axis strut must be selected.";
                const blueOrbit: Direction = this.symmetry.getDirection("blue");
                const blueScale: AlgebraicNumber = blueOrbit.getUnitLength();
                o0 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 2).normal().scale(blueScale);
                o1 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 54).normal().scale(blueScale);
                o2 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 36).normal().scale(blueScale);
                const redOrbit: Direction = this.symmetry.getDirection("red");
                const redScale: AlgebraicNumber = redOrbit.getUnitLength();
                n0 = redOrbit.getAxis$int$int(Symmetry.PLUS, 2).normal().scale(redScale);
                n1 = redOrbit.getAxis$int$int(Symmetry.PLUS, 46).normal().scale(redScale);
                n2 = redOrbit.getAxis$int$int(Symmetry.PLUS, 16).normal().scale(redScale);
                break;
            };
        case "red":
            {
                ...
                const blueScale: AlgebraicNumber = blueOrbit.getUnitLength();
                const redOrbit: Direction = this.symmetry.getDirection("red");
                let redScale: AlgebraicNumber = redOrbit.getUnitLength();
                if (this.first){
                    o0 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 56).normal().scale(blueScale);
                    o1 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 38).normal().scale(blueScale);
                    o2 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 40).normal().scale(blueScale);
                    n0 = redOrbit.getAxis$int$int(Symmetry.PLUS, 46).normal().scale(redScale);
                    n1 = redOrbit.getAxis$int$int(Symmetry.PLUS, 1).normal().scale(redScale);
                    n2 = redOrbit.getAxis$int$int(Symmetry.PLUS, 2).normal().scale(redScale);
                } else {
                    o0 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 37).normal().scale(blueScale);
                    o1 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 25).normal().scale(blueScale);
                    o2 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 45).normal().scale(blueScale);
                    n0 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 37).normal().scale(blueScale);
                    n1 = blueOrbit.getAxis$int$int(Symmetry.PLUS, 25).normal().scale(blueScale);
                    redScale = redScale['times$com_vzome_core_algebra_AlgebraicNumber'](this.symmetry.getField()['createPower$int'](-1));
                    n2 = redOrbit.getAxis$int$int(Symmetry.PLUS, 45).normal().scale(redScale);
                }
                break;
            };
        default:
        ...
        if (prepareTool){
            const orientation: AlgebraicMatrix = this.symmetry.getMatrix(zone.getOrientation());
            const inverse: AlgebraicMatrix = orientation.inverse();
            let oldBasis: AlgebraicMatrix = new AlgebraicMatrix(o0, o1, o2);
            let newBasis: AlgebraicMatrix = new AlgebraicMatrix(n0, n1, n2);
            if (this.stretch){
                const temp: AlgebraicMatrix = oldBasis;
                oldBasis = newBasis;
                newBasis = temp;
            }
            const matrix: AlgebraicMatrix = orientation.times(newBasis.times(oldBasis.inverse()).times(inverse));
            this.transforms = [null];
            this.transforms[0] = new MatrixTransformation(matrix, center.getLocation());
        }
        return null;
    }
    ...
    setXmlAttributes(element: Element, format: XmlSaveFormat) {
        ...
        this.symmetry = <IcosahedralSymmetry><any>(<XmlSymmetryFormat>format).parseSymmetry("icosahedral");
...
export namespace AxialStretchTool {
    ...
    export class Factory extends AbstractToolFactory {
        red: boolean;
        ...
        static getToolTip(red: boolean, stretch: boolean, first: boolean): string {
            if (red)if (first)return stretch ? AxialStretchTool.TOOLTIP_REDSTRETCH1 : AxialStretchTool.TOOLTIP_REDSQUASH1; else return stretch ? AxialStretchTool.TOOLTIP_REDSTRETCH2 : AxialStretchTool.TOOLTIP_REDSQUASH2; else return stretch ? AxialStretchTool.TOOLTIP_YELLOWSTRETCH : AxialStretchTool.TOOLTIP_YELLOWSQUASH;
        }

        public constructor(tools: ToolsModel, symmetry: IcosahedralSymmetry, red: boolean, stretch: boolean, first: boolean) {
        ...
        public createToolInternal(id: string): Tool {
            ...
            return new AxialStretchTool(id, <IcosahedralSymmetry><any>this.getSymmetry(), this.getToolsModel(), this.stretch, this.red, this.first, category);
        }
        ...
        bindParameters(selection: Selection): boolean {
            const symmetry: IcosahedralSymmetry = <IcosahedralSymmetry><any>this.getSymmetry();
            for(let index=selection.iterator();index.hasNext();) {
                ...
                if (man != null && (man.constructor != null && man.constructor["__interfaces"] != null && man.constructor["__interfaces"].indexOf("com.vzome.core.model.Strut") >= 0)){
                    const axisStrut: Strut = <Strut><any>man;
                    let vector: AlgebraicVector = axisStrut.getOffset();
                    vector = symmetry.getField().projectTo3d(vector, true);
                    const axis: Axis = symmetry.getAxis$com_vzome_core_algebra_AlgebraicVector(vector);
