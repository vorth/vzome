...
import { ZomicNamingConvention } from "./ZomicNamingConvention.js";
...
export class ZomicCompilerState {
    public constructor(icosaSymm: IcosahedralSymmetry) {
        ...
        this.namingConvention = new ZomicNamingConvention(icosaSymm);
    }

    /*private*/ icosaSymmetry: IcosahedralSymmetry;

    /*private*/ namingConvention: ZomicNamingConvention;

    /*private*/ statements: java.util.Stack<ZomicStatement>;

    /*private*/ templates: java.util.Stack<ZomicCompilerState.ZomicStatementTemplate<any>>;

    public getProgram(): Walk {
        return this.statements.size() === 0 ? new Walk() : <Walk>this.statements.firstElement();
    }

    public prepareStatement(statement: ZomicStatement) {
    ...
    public commitLastStatement() {
        const statement: ZomicStatement = this.statements.pop();
        if (statement != null && statement instanceof <any>Nested){
            const body: ZomicStatement = (<Nested>statement).getBody();
            if ((body == null) || ((body != null && body instanceof <any>Walk) && (<Walk>body).size() === 0)){
                return;
            }
        }
    ...
    public commit(newStatement: ZomicStatement) {
        const currentStatement: ZomicStatement = this.statements.peek();
        if (currentStatement != null && currentStatement instanceof <any>Walk){
...
export namespace ZomicCompilerState {
    ...
    export interface ZomicStatementTemplate<T extends ZomicStatement> {
        generate(): T;
    }
    ...
    export class AxisInfo implements ZomicCompilerState.IHaveAxisInfo {
        ...
        generate(): Axis {
            try {
                const axis: Axis = this.__parent.namingConvention.getAxis(this.__axisColor, this.indexFullName());
                if (axis == null){
    ...
    export class ScaleInfo {
        ...
        generate(symmetry: IcosahedralSymmetry): AlgebraicNumber {
    ...
    export class RotateTemplate implements ZomicCompilerState.ZomicStatementTemplate<Rotate>, ZomicCompilerState.IHaveAxisInfo {
        public __parent: any;
         ...
         * @return {Rotate}
         */
        public generate(): Rotate {
            const axis: Axis = this.axisInfo.generate();
            return new Rotate(axis, this.steps);
        }
    ...
    export class ReflectTemplate implements ZomicCompilerState.ZomicStatementTemplate<Reflect>, ZomicCompilerState.IHaveAxisInfo {
        public __parent: any;
        axisInfo: ZomicCompilerState.AxisInfo;
         ...
         * @return {Reflect}
         */
        public generate(): Reflect {
            const result: Reflect = new Reflect();
    ...
    export class SymmetryTemplate implements ZomicCompilerState.ZomicStatementTemplate<Symmetry>, ZomicCompilerState.IHaveAxisInfo {
        public __parent: any;
        axisInfo: ZomicCompilerState.AxisInfo;
         ...
         * @return {Symmetry}
         */
        public generate(): Symmetry {
            const result: Symmetry = <Symmetry>this.__parent.statements.peek();
            switch((this.symmetryMode)) {
            case ZomicCompilerState.SymmetryModeEnum.Icosahedral:
                break;
            case ZomicCompilerState.SymmetryModeEnum.RotateAroundAxis:
                {
                    const rotate: Rotate = new Rotate(null, -1);
                    const axis: Axis = this.axisInfo.generate();
                    rotate.setAxis(axis);
                    result.setPermute(rotate);
                };
                break;
            case ZomicCompilerState.SymmetryModeEnum.MirrorThroughBlueAxis:
                {
                    const reflect: Reflect = new Reflect();
                    const axis: Axis = this.axisInfo.generate();
                    reflect.setAxis(axis);
                    result.setPermute(reflect);
                };
                break;
            case ZomicCompilerState.SymmetryModeEnum.ReflectThroughOrigin:
                result.setPermute(new Reflect());
                break;
            default:
                throw new java.lang.IllegalStateException("Unexpected SymmetryModeEnum: " + this.symmetryMode == null ? "<null>" : ZomicCompilerState.SymmetryModeEnum["_$wrappers"][this.symmetryMode].toString());
            }
            return result;
        }
    ...
    export class ScaleTemplate extends ZomicCompilerState.ScaleInfo implements ZomicCompilerState.ZomicStatementTemplate<Scale> {
        public __parent: any;
        ...
        public generate$(): Scale {
            const algebraicNumber: AlgebraicNumber = this.generate(this.__parent.icosaSymmetry);
    ...
    export class MoveTemplate extends ZomicCompilerState.ScaleInfo implements ZomicCompilerState.ZomicStatementTemplate<Move>, ZomicCompilerState.IHaveAxisInfo {
        public __parent: any;
        axisInfo: ZomicCompilerState.AxisInfo;
        ...
        public constructor(__parent: any) {
            ...
            this.scale = ZomicNamingConvention.MEDIUM;
        }
        ...
        public generate$java_lang_String(axisColor: string): AlgebraicNumber {
            if (this.denominator !== 1){
                const direction: Direction = this.__parent.icosaSymmetry.getDirection(axisColor);
        ...
        public generate(axisColor?: any): any {
            ...
            } else if (((axisColor != null && axisColor instanceof <any>IcosahedralSymmetry) || axisColor === null)) {
        ...
        public generate$(): Move {
            const axis: Axis = this.axisInfo.generate();
            const strutLength: AlgebraicNumber = this.generate$java_lang_String(this.axisColor());
