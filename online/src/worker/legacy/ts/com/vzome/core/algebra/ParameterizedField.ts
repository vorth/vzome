import { AbstractAlgebraicField } from "./AbstractAlgebraicField.js";
 ...
 * @param {*} factory
 * @class
 * @extends AbstractAlgebraicField
 */
export abstract class ParameterizedField extends AbstractAlgebraicField {
    coefficients: number[];
     ...
     * @param {*} field
     */
    static doNothing(field: AlgebraicField, factors: BigRational[]) {
    }
     ...
     */
    normalizer: (p1: AlgebraicField, p2: BigRational[]) => void;
    ...
    /**
     * 
     * @param {BigRational[]} v1
     * @param {BigRational[]} v2
     * @return {BigRational[]}
     */
    multiply(v1: BigRational[], v2: BigRational[]): BigRational[] {
        const order: number = this.getOrder();
        const result: BigRational[] = (s => { let a=[]; while(s-->0) a.push(null); return a; })(order);
        for(let i: number = 0; i < order; i++) {{
            ...
            for(let j: number = 0; j < order; j++) {{
                for(let k: number = 0; k < order; k++) {{
                    ...
                    if (multiplier !== 0){
                        let product: BigRational = v1[j].times(v2[k]);
                        if (multiplier !== 1){
     ...
     * @return {BigRational[]}
     */
    scaleBy(factors: BigRational[], whichIrrational: number): BigRational[] {
     ...
     * @return {number}
     */
    evaluateNumber(factors: BigRational[]): number {
