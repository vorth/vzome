 ...
 * @class
 */
export class RenderedManifestation implements RenderedObject {
    /*private*/ mManifestation: Manifestation;

    /*private*/ mShape: Polyhedron;

    /*private*/ color: Color;

    /*private*/ mOrientation: AlgebraicMatrix;
    ...
    /*private*/ location: AlgebraicVector;

    /*private*/ fixedLocation: AlgebraicVector;

    /*private*/ strutZone: number;

    /*private*/ strutLength: AlgebraicNumber;

    /*private*/ strutOrbit: Direction;
    ...
    public constructor(m: Manifestation, orbitSource: OrbitSource) {
    ...
    public getManifestation(): Manifestation {
    ...
    public setOrientation(m: AlgebraicMatrix) {
        this.mOrientation = m;
    }

    public getOrientation(): AlgebraicMatrix {
        return this.mOrientation;
    }

    public getLocation(): RealVector {
        if (this.location != null)return this.getEmbedding().embedInR3(this.location); else return new RealVector(0.0, 0.0, 0.0);
    }

    public getLocationAV(): AlgebraicVector {
    ...
    public setStrut(orbit: Direction, zone: number, sense: number, length: AlgebraicNumber) {
    ...
    public getStrutLength(): AlgebraicNumber {
        return this.strutLength;
    }

    public getStrutOrbit(): Direction {
        return this.strutOrbit;
    }

    offsetLocation() {
        if (this.mManifestation != null){
            const strut: Strut = <Strut><any>this.mManifestation;
    ...
    public resetAttributes(oneSidedPanels: boolean, colorPanels: boolean) {
        ...
        if (this.mManifestation != null && (this.mManifestation.constructor != null && this.mManifestation.constructor["__interfaces"] != null && this.mManifestation.constructor["__interfaces"].indexOf("com.vzome.core.model.Panel") >= 0)){
            ...
            this.resetConnectorAttributes(<Connector><any>this.mManifestation);
    ...
    /*private*/ resetPanelAttributes(oneSidedPanels: boolean, colorPanels: boolean) {
        const shapes: Shapes = this.orbitSource.getShapes();
        const panel: Panel = <Panel><any>this.mManifestation;
        this.location = panel.getFirstVertex();
        const relativeVertices: java.util.List<AlgebraicVector> = <any>(new java.util.ArrayList<any>());
        ...
        const zone: Axis = this.orbitSource.getAxis(normal);
        const shape: Polyhedron = shapes.getPanelShape(panel.getVertexCount(), panel.getQuadrea(), zone, relativeVertices, oneSidedPanels);
        ...
        const orientation: AlgebraicMatrix = shapes.getSymmetry().getMatrix(orn);
        ...
        try {
            ...
            let color: Color = this.mManifestation.getColor();
            if (color == null){
