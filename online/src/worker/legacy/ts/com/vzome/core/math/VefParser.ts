...
export abstract class VefParser {
    ...
    parsedOffset: AlgebraicVector;

    abstract startVertices(numVertices: number);

    abstract addVertex(index: number, location: AlgebraicVector);
    ...
    public parseVEF(vefData: string, field: AlgebraicField) {
        this.field = field;
        ...
        const scaleVector: AlgebraicVector = new AlgebraicVector(field, this.dimension);
        if (token === ("scale")){
            try {
                token = tokens.nextToken();
                if (token === ("vector")){
                    try {
                        for(let tokNum: number = 0; tokNum < this.dimension; tokNum++) {{
                            token = tokens.nextToken();
                            const coord: AlgebraicNumber = this.field.parseVefNumber(token, this.__isRational);
                    ...
                    const scale: AlgebraicNumber = this.field.parseVefNumber(token, this.__isRational);
        ...
        this.parsedOffset = new AlgebraicVector(field, this.dimension);
        if (token === ("offset")){
            try {
                for(let tokNum: number = 0; tokNum < this.dimension; tokNum++) {{
                    token = tokens.nextToken();
        ...
        for(let i: number = 0; i < numVertices; i++) {{
            let v: AlgebraicVector = field.origin(this.dimension);
            for(let tokNum: number = 0; tokNum < this.dimension; tokNum++) {{
                try {
                    token = tokens.nextToken();
                } catch(e1) {
                ...
                const coord: AlgebraicNumber = this.field.parseVefNumber(token, this.__isRational)['times$com_vzome_core_algebra_AlgebraicNumber'](scaleVector.getComponent(tokNum));
