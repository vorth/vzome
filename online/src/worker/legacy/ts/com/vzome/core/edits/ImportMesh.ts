import { java, javaemul } from "../../../../../candies/j4ts-2.1.0-SNAPSHOT/bundle.js";
import { AlgebraicField } from "../algebra/AlgebraicField.js";
import { AlgebraicNumber } from "../algebra/AlgebraicNumber.js";
import { AlgebraicVector } from "../algebra/AlgebraicVector.js";
import { Command } from "../commands/Command.js";
import { XmlSaveFormat } from "../commands/XmlSaveFormat.js";
import { Point } from "../construction/Point.js";
import { ChangeManifestations } from "../editor/api/ChangeManifestations.js";
import { EditorModel } from "../editor/api/EditorModel.js";
import { ManifestConstructions } from "../editor/api/ManifestConstructions.js";
import { PerspectiveProjection } from "../math/PerspectiveProjection.js";
import { Projection } from "../math/Projection.js";
import { QuaternionProjection } from "../math/QuaternionProjection.js";
import { SixCubeProjection } from "../math/SixCubeProjection.js";
import { TetrahedralProjection } from "../math/TetrahedralProjection.js";
import { Connector } from "../model/Connector.js";
import { DomUtils } from "../../xml/DomUtils.js";
import { Element } from "../../../../org/w3c/dom/Element.js";
import { Node } from "../../../../org/w3c/dom/Node.js";

export abstract class ImportMesh extends ChangeManifestations {
    meshData: string;

    scale: AlgebraicNumber;

    projection: Projection;

    editor: EditorModel;

    public constructor(editor: EditorModel) {
        super(editor);
        if (this.meshData === undefined) { this.meshData = null; }
        if (this.scale === undefined) { this.scale = null; }
        if (this.projection === undefined) { this.projection = null; }
        if (this.editor === undefined) { this.editor = null; }
        this.editor = editor;
    }

    /**
     * 
     * @param {*} params
     */
    public configure(params: java.util.Map<string, any>) {
        const field: AlgebraicField = this.mManifestations.getField();
        this.meshData = <string>params.get("vef");
        this.projection = <Projection><any>params.get("projection");
        this.scale = <AlgebraicNumber><any>params.get("scale");
        if (this.scale == null){
            this.scale = field.one();
        }
        const mode: string = <string>params.getOrDefault("mode", "");
        switch((mode)) {
        case "":
            break;
        case "clipboard":
            if (this.meshData != null && !/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(this.meshData, "vZome VEF") && !/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(this.meshData, "{"))this.meshData = null;
            break;
        case "Quaternion":
            break;
        default:
            this.setProjection(mode, field);
            break;
        }
    }

    /**
     * 
     * @return {boolean}
     */
    public isNoOp(): boolean {
        return this.meshData == null;
    }

    /**
     * 
     * @param {*} element
     */
    getXmlAttributes(element: Element) {
        if (this.scale != null)DomUtils.addAttribute(element, "scale", this.scale.toString(AlgebraicField.ZOMIC_FORMAT));
        if (this.projection != null){
            const name: string = this.projection.getProjectionName();
            if (!("" === name)){
                DomUtils.addAttribute(element, "projection", this.projection.getProjectionName());
                this.projection.getXmlAttributes(element);
            }
        }
        const textNode: Node = element.getOwnerDocument().createTextNode(XmlSaveFormat.escapeNewlines(this.meshData));
        element.appendChild(textNode);
    }

    /**
     * 
     * @param {*} xml
     * @param {XmlSaveFormat} format
     */
    setXmlAttributes(xml: Element, format: XmlSaveFormat) {
        const field: AlgebraicField = format.getField();
        this.scale = format.parseRationalNumber(xml, "scale");
        if (this.scale == null){
            this.scale = field.one();
        }
        this.meshData = xml.getTextContent();
        const projectionName: string = xml.getAttribute("projection");
        if ("" === projectionName){
            const quaternion: AlgebraicVector = format.parseRationalVector(xml, "quaternion");
            this.projection = quaternion == null ? null : new QuaternionProjection(field, null, quaternion);
        } else {
            this.setProjection(projectionName, field);
        }
        if (this.projection != null){
            this.projection.setXmlAttributes(xml);
        }
    }

    setProjection(projectionName: string, field: AlgebraicField) {
        switch((projectionName)) {
        case "Quaternion":
            this.projection = new QuaternionProjection(field, null, null);
            break;
        case "SixCube":
            this.projection = new SixCubeProjection(field);
            break;
        case "Tetrahedral":
            this.projection = new TetrahedralProjection(field);
            break;
        case "Perspective":
            this.projection = new PerspectiveProjection(field, null);
            break;
        }
    }

    /**
     * 
     */
    public perform() {
        if (this.meshData == null)return;
        let offset: AlgebraicVector = null;
        let pointFound: boolean = false;
        for(let index=this.mSelection.iterator();index.hasNext();) {
            let man = index.next();
            {
                if (man != null && (man.constructor != null && man.constructor["__interfaces"] != null && man.constructor["__interfaces"].indexOf("com.vzome.core.model.Connector") >= 0)){
                    const nextPoint: Point = <Point>(<Connector><any>man).getFirstConstruction();
                    if (!pointFound){
                        pointFound = true;
                        offset = nextPoint.getLocation();
                    } else {
                        offset = null;
                        break;
                    }
                }
                if (this.deselectInputs())this.unselect$com_vzome_core_model_Manifestation(man);
            }
        }
        const field: AlgebraicField = this.mManifestations.getField();
        const events: ManifestConstructions = new ManifestConstructions(this);
        const registry: AlgebraicField.Registry = new ImportMesh.ImportMesh$0(this, field);
        if (this.projection == null)this.projection = new Projection.Default(field);
        try {
            this.parseMeshData(offset, events, registry);
        } catch(e) {
            throw new Command.Failure("Incorrect content for this import:\n" + e.message);
        }
        this.redo();
    }

    abstract parseMeshData(offset: AlgebraicVector, events: ManifestConstructions, registry: AlgebraicField.Registry);

    deselectInputs(): boolean {
        return true;
    }
}
ImportMesh["__class"] = "com.vzome.core.edits.ImportMesh";


export namespace ImportMesh {

    export class ImportMesh$0 implements AlgebraicField.Registry {
        public __parent: any;
        /**
         * 
         * @param {string} name
         * @return {*}
         */
        public getField(name: string): AlgebraicField {
            if (this.field.supportsSubfield(name))return this.field; else {
                return null;
            }
        }

        constructor(__parent: any, private field: any) {
            this.__parent = __parent;
        }
    }
    ImportMesh$0["__interfaces"] = ["com.vzome.core.algebra.AlgebraicField.Registry"];


}
