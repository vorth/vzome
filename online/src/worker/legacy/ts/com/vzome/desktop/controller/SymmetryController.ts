...
import { SymmetrySystem } from "../../core/editor/SymmetrySystem.js";
...
import { OrbitSetController } from "./OrbitSetController.js";
import { OrbitSnapper } from "./OrbitSnapper.js";
import { SymmetrySnapper } from "./SymmetrySnapper.js";
import { ToolFactoryController } from "./ToolFactoryController.js";
...
export class SymmetryController extends DefaultController {
    ...
    public getProperty(string: string): string {
        ...
        default:
            if (/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(string, "orbitColor.")){
                ...
                const dir: Direction = this.symmetrySystem.getOrbits().getDirection(name);
    ...
    public availableOrbits: OrbitSet;

    public snapOrbits: OrbitSet;

    public buildOrbits: OrbitSet;

    public renderOrbits: OrbitSet;

    /*private*/ snapper: OrbitSnapper;

    public availableController: OrbitSetController;

    public snapController: OrbitSetController;

    public buildController: OrbitSetController;

    public renderController: OrbitSetController;

    public orbitLengths: java.util.Map<Direction, Controller>;

    /*private*/ symmetryToolFactories: java.util.Map<string, Controller>;

    /*private*/ transformToolFactories: java.util.Map<string, Controller>;

    /*private*/ linearMapToolFactories: java.util.Map<string, Controller>;
    ...
    public getSnapper(): OrbitSnapper {
        return this.snapper;
    }

    public constructor(label: string, parent: Controller, model: SymmetrySystem, mRenderedModel: RenderedModel) {
        super();
        ...
        const symmetry: Symmetry = model.getSymmetry();
        this.availableOrbits = new OrbitSet(symmetry);
        this.snapOrbits = new OrbitSet(symmetry);
        this.buildOrbits = new OrbitSet(symmetry);
        this.renderOrbits = new OrbitSet(symmetry);
        this.snapper = new SymmetrySnapper(this.snapOrbits);
        ...
        this.availableController = new OrbitSetController(this.availableOrbits, this.symmetrySystem.getOrbits(), this.symmetrySystem);
        this.addSubController("availableOrbits", this.availableController);
        this.snapController = new OrbitSetController(this.snapOrbits, this.availableOrbits, this.symmetrySystem);
        this.addSubController("snapOrbits", this.snapController);
        this.buildController = new OrbitSetController(this.buildOrbits, this.availableOrbits, this.symmetrySystem);
        ...
        this.renderController = new OrbitSetController(this.renderOrbits, this.symmetrySystem.getOrbits(), this.symmetrySystem);
        ...
        for(let index=this.symmetrySystem.getOrbits().getDirections().iterator();index.hasNext();) {
            let orbit = index.next();
            {
                const unitLength: AlgebraicNumber = this.symmetrySystem.getOrbitUnitLength(orbit);
                const field: AlgebraicField = symmetry.getField();
                const lengthModel: Controller = new LengthController(unitLength, field.one(), field);
    ...
    public getCommandList(listName: string): string[] {
        ...
        case "symmetryToolFactories":
            if (this.symmetryToolFactories.isEmpty()){
                for(let index=this.symmetrySystem.getToolFactories(Tool.Kind.SYMMETRY).iterator();index.hasNext();) {
        ...
        case "transformToolFactories":
            if (this.transformToolFactories.isEmpty()){
                for(let index=this.symmetrySystem.getToolFactories(Tool.Kind.TRANSFORM).iterator();index.hasNext();) {
                    let factory = index.next();
        ...
        case "linearMapToolFactories":
            if (this.linearMapToolFactories.isEmpty()){
                for(let index=this.symmetrySystem.getToolFactories(Tool.Kind.LINEAR_MAP).iterator();index.hasNext();) {
                    let factory = index.next();
        ...
        case "builtInSymmetryTools":
            ...
            for(let index=this.symmetrySystem.getPredefinedTools(Tool.Kind.SYMMETRY).iterator();index.hasNext();) {
                let tool = index.next();
        ...
        case "builtInTransformTools":
            ...
            for(let index=this.symmetrySystem.getPredefinedTools(Tool.Kind.TRANSFORM).iterator();index.hasNext();) {
                let tool = index.next();
    ...
    public getSubController(name: string): Controller {
        ...
        default:
            if (/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(name, "length.")){
                ...
                const dir: Direction = this.symmetrySystem.getOrbits().getDirection(dirName);
                return this.getLengthController(dir);
            }
            let result: Controller = this.symmetryToolFactories.get(name);
    ...
    /*private*/ getLengthController(orbit: Direction): Controller {
        let result: Controller = this.orbitLengths.get(orbit);
        if (result == null && orbit != null){
            const unitLength: AlgebraicNumber = this.symmetrySystem.getOrbitUnitLength(orbit);
            const field: AlgebraicField = this.symmetrySystem.getSymmetry().getField();
    ...
    public getZone(offset: AlgebraicVector): Axis {
    ...
    public getBuildOrbits(): OrbitSet {
    ...
    public parseOrbitLength(string: string, props: java.util.Map<string, any>) {
        ...
        const orbit: Direction = this.getOrbits().getDirection(orbitStr);
        ...
        if (/* endsWith */((str, searchString) => { let pos = str.length - searchString.length; let lastIndex = str.indexOf(searchString, pos); return lastIndex !== -1 && lastIndex === pos; })(mode, "Struts")){
            ...
            const unitScalar: AlgebraicNumber = orbit.getLengthInUnits(this.getSymmetry().getField().one());
            const length: AlgebraicNumber = unitScalar.getField().parseNumber(lengthStr);
            const rawLength: AlgebraicNumber = length.dividedBy(unitScalar);
...
export namespace SymmetryController {
    ...
    export class SymmetryController$0 implements PropertyChangeListener {
        public __parent: any;
        /**
         * 
